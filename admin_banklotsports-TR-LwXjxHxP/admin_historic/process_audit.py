...
from admin_juego.models import Encuentros, EncuentrosDetail, EncuentrosModalidades, Jugadas, Modalidades
...
class AdminJuegoProcess(object):
    ...
    def process_encuentros(self, objecto):
        ...
        if objecto.json.get('process') == 'create':
            ...
            json['object']['fields_related']['Equipos'] = equipos
            ...
            equipos = {}
            model = EncuentrosDetail

            for refr in ref_relateds:
                fields = get_fields_basic_update(model, refr.json['attr'])
                if fields:
                    nombre = EncuentrosDetail.objects.get(
                        pk=refr.json['object']
                    )

                    fields[0]['Home/Visitante'][0] = get_model(
                        'admin_juego',
                        'encuentrosdetail'
                    ).str_indice[fields[0]['Home/Visitante'][0]]

                    fields[0]['Home/Visitante'][1] = get_model(
                        'admin_juego',
                        'encuentrosdetail'
...
class AdminComercializacionProcess(object):
    ...
    def humanize_permissionssalesrestrictions(self, dic):
        ...
        strs = '<table class="table">'
        for key in dic.keys():
            modalidad = Modalidades.objects.get(pk=key)
            values = ''
            for value in dic[key]:
                modalidad = Modalidades.objects.get(pk=value)
...
class ProcessModelGeneric(object):
    ...
    def process(self, objecto):
        model = get_model(objecto.get_app(), objecto.get_model())
