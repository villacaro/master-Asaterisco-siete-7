...
from admin_banklotsports.settings import ACCESO_URL, INDEX_URL, LOGOUT_URL, PAGE_404_URL, REDIS_DB
...
class AuthenticationAndPermissionsMiddleware(object):
    ...
    def process_request(self, request):
        ...
        if ignore_links(request.path) is False:
            # De estar en un link que no debe ser ingnorado se verifica la variable de session
            if settings.SESSION_COOKIE_KEY in request.session:
                pass
            else:
                return self._redirect(request.path, '')  # "No existe la variable de session")
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if ignore_links(request.path) is False:
            # Estando en urls ignoradas, procedo a generar la data global en los kwargs
            ...
            try:
                ...
                if not view_kwargs["object_session"].check_seccion():
                    return self._redirect(request.path, '')  # "Session invalida ")
            except Exception:
                return self._redirect(request.path, '')  # "Excepiton Session invalida")
            ...
            try:
                ...
                return self._redirect(request.path, '')  # "Exception obteniendo user_ip")
            ...
            try:
                ...
                return self._redirect(request.path, '')  # "Exception obteniendo user")

            # objeto de la comercializadora
            try:
                view_kwargs["object_comercializadora"] = obj.get_comercializadora(request)
            except Exception:
            ...
            if view_kwargs["object_user"].profile.codename != "userprofile_master":
                access = self.get_access_panel(view_kwargs["object_comercializadora"])
                if not access:
                    return self._redirect(request.path, "Sistema en mantenimiento")

            # objeto del sistema de juego asociado
            try:
                view_kwargs["object_sistema_juego"] = obj.get_sistemaJuego(request)
            except Exception:
    ...
    def _add_info_menu(self, request, response):
        ...
        if response.context_data and "view" in response.context_data:
            ...
            if exits is True:
                ...
                # Ejecuta el algoritmo de consulta para de los permisos en el objeto de users
                user = response.context_data["view"].object_user
            ...
            if exits is True:
                ...
                if response.context_data["view"].object_sistema_juego:
                    ...
                    if response.context_data["view"].object_sistema_juego.pk == response.context_data[
                        ...
                        querryset = EventNotification.objects.filter(
                            sistema=response.context_data["view"].object_sistema_juego.pk,
                            in_production=False
                        )
                    else:
                        querryset = EventNotification.objects.filter(
                            sistema__in=[
                                response.context_data["view"].object_sistema_juego.pk,
                                response.context_data["view"].object_sistema_logros.pk
                            ],
                            in_production=False
                        )
                ...
                if response.context_data["view"].object_comercializadora:
                    ...
                    querryset = MessageComer.objects.filter(
                        comercializadora_id=response.context_data["view"].object_comercializadora.pk,
                        read=False,
    ...
    def get_access_panel(self, comercializadora):
        ...
        if REDIS_DB.get('PANEL_MAINTENANCE_GLOBAL') == b'0':
            ...
            if origen:
                ...
                key_redis = 'PANEL_MAINTENANCE_GLOBAL-{0}'.format(comercializadora.id)
                value = REDIS_DB.get(key_redis)
                if value == b'0':
                    access = False
        return access
