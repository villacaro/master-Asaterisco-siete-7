...
from admin_juego.models import (
    Encuentros, EncuentrosDetail, Equipos, EquiposTemporadas, Jugador, JugadorTipo, Temporadas,
)
...
class EncuentrosCreateView(EncuentrosView, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(equipos) > 0:
            indices = {}
            jugadores = {}
            temporada = Temporadas.objects.get(
                pk=temporada
            )

            referencias = False
            for obj in equipos:
                equipo = Equipos.objects.get(
                    pk=obj
                )
                ...
                equipo_temporada = EquiposTemporadas.objects.get_or_create(
                    temporada=temporada,
                    equipo=equipo
                )[0]
                ...
                equipo_choques = EncuentrosDetail.objects.filter(
                ...
                try:
                    ...
                    jugador = EncuentrosDetail.objects.filter(
                        jugador_id=jugador,
            ...
            jugadores_exist = Jugador.objects.filter(
        ...
        if len(equipos) > 0:
            for obj in equipos:
                ...
                try:
                    ...
                    equipo_temporada = EquiposTemporadas.objects.get_or_update(
                        temporada=form.instance.jornada.temporadas,
                        equipo=equipo
                    )[0]
                ...
                EncuentrosDetail.objects.create(
    ...
    def get_queryset(self):
        ...
        return encuentros.filter(
            horajuego__gte=(
                now() -
                timedelta(
                    days=Encuentros.maximo_dias_olgura)).date()
        )

    def get_context_data(self, **kwargs):
        context = super(EncuentrosUpdateView, self).get_context_data(**kwargs)
        equipos = EncuentrosDetail.objects.select_related('equipos_temporadas__equipo').filter(
        ...
        for equipo in equipos:
            ...
            if equipo.jugador is not None:
                ...
                equipo_json['jugadores'] = JugadorTipo.objects.filter(
    ...
    def form_valid(self, form):
        ...
        if len(equipos) > 0:
            ...
            for obj in equipos:
                ...
                referencia = self.request.POST.get('referencia_' + str(obj))
                detalle_encuentro = EncuentrosDetail.objects.get_or_create(
                    encuentro=form.instance,
                    equipos_temporadas=equipo_temporada
                )[0]
            ...
            for old_detalle_encuentro in EncuentrosDetail.objects.filter(
                encuentro=form.instance
            ).exclude(
                pk__in=new_equipos
            ):
...
class EncuentrosListbyTemporadaAjax(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        def label_from_instance_encuentros(obj):
            ...
            campo = '{0} Fecha: {1} {2} Equipos: '.format(
                obj.pk,
                objFecha.getFecha(),
                objFecha.getHora(),
            )
