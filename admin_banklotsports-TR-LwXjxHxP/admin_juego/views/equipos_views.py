...
from admin_juego.models import Equipos, EquiposLigas, EquiposTemporadas, GruposJuego, Temporadas, Torneos
    ...
    def form_valid(self, form):
        ...
        if len(ligas) > 0:
            for i, liga in enumerate(ligas):
                EquiposLigas.objects.create(
                    equipo=form.instance,
                    liga=Torneos.objects.get(pk=liga)
                )
        if self.request.POST['_save'] == '_addanother':
            ...
            for liga in ligas:
                ...
                if EquiposLigas.objects.filter(
                    equipo=form.instance,
                    liga=liga
                ).exists():
...
class EquiposListView(EquiposView, ListView):
    ...
    def get_queryset(self):
        equipos = Equipos.objects.select_related('deporte').all()
...
class EquiposUpdateView(EquiposView, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if len(ligas) > 0:
            new_equipo_liga = []
            for i, liga in enumerate(ligas):
                equipo_liga = EquiposLigas.objects.get_or_create(
                    equipo=form.instance,
                    liga=Torneos.objects.get(pk=liga)
                )[0]
                new_equipo_liga.append(equipo_liga.pk)

            for old_equipo_liga in EquiposLigas.objects.filter(
                equipo=form.instance
...
class EquiposListbyTemporadaAjax(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if EquiposTemporadas.objects.filter(
            temporada=temporada
        ).exists():
...
class EquiposListbyGrupoAjax(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        except GruposJuego.DoesNotExist:
            pass
