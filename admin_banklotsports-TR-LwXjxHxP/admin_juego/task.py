...
from admin_juego.models import EventNotification, Temporadas
...
class AsyncCreateAutomaticTemporadas(AsyncGestionOperationalError):
    ...
    def run_try(self, *args, **kwargs):
        ...
        count = 0
        for last in Temporadas.objects.filter(fechafin=today):
            nombre = 'Temporada {0}'.format(year + 1)
            temp = Temporadas.objects.filter(nombre=nombre, torneo_id=last.torneo.id)
            if not temp.exists():
                count += 1
                Temporadas.objects.create(
                    nombre=nombre,
