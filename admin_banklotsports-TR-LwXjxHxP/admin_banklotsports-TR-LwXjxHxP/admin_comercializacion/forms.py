...
from admin_juego.models import Deportes, SistemaJuego
    ...
    def __init__(self, *args, **kwargs):
        ...
        if instance:
            kwargs.update(initial={
                'monto_diario': 0.00 if not instance.monto_diario else instance.monto_diario,
                'monto_premio': 0.00 if not instance.monto_premio else instance.monto_premio,
            })
...
class PermissionsSalesForm(WidgetCustomizeForms, forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        deportes = Deportes.objects.only('pk', 'nombre').all().order_by('orden')
...
class PermissionsSalesRestrictionsForm(WidgetCustomizeForms, forms.Form):
    deporte = forms.ModelChoiceField(
        required=True,
        queryset=Deportes.objects.all().only('nombre'),
                    ...
                    if isinstance(value, str) \
                        ...
                        if preference.comparison != TypePreferences.comparison_codenames['codename_free']:
                            ...
                            if self.type != 'userprofile_bloque':
                                ...
                                value_limit = Decimal(self.object.get_preference_value_by_codename(
                                    preference.codename
                                ))
