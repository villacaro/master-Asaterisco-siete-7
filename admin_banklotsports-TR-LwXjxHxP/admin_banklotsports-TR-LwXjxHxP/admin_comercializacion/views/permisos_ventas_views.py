...
from admin_juego.models import Deportes, Deportes_Grupos, Modalidades, Modalidades_Grupos
...
class PermissionsSalesFormView(PermissionsSalesView, FormView):
    ...
    def process_permissions_sales(self, objecto, datos):
        ...
        deportes_check = Deportes.objects.only('pk').filter(pk__in=deportes_list)

        deportes_nocheck = Deportes.objects.only('pk').all().exclude(pk__in=deportes_list)
        ...
        for obj in deportes_check:
            ...
            if grupos_list:
                grupos_check = Deportes_Grupos.objects.select_related('grupo').filter(
                    deporte_id=obj.id, grupo_id__in=grupos_list)

                grupos_nocheck = Deportes_Grupos.objects.select_related('grupo').filter(
                    deporte_id=obj.id,
                ).exclude(
                ...
                for grupo in grupos_check:
                    ...
                    if modalidades_list:
                        modalidades_check = Modalidades_Grupos.objects.select_related('modalidad')\
                            .filter(grupo_id=grupo.grupo.id, modalidad_id__in=modalidades_list)

                        modalidades_nocheck = Modalidades_Grupos.objects.select_related('modalidad')\
    ...
    def save_permissions(self, objecto, json):
        ...
        for add in json_add:
            ...
            if not add.get('grupo_id') and not add.get('modalidad_id'):
                ...
                json_data['data']['pk_origin'] = Deportes_Grupos.objects.only('pk').get(
                ...
                json_data['data']['pk_origin'] = Modalidades_Grupos.objects.only('pk').get(
                    grupo_id=add.get('grupo_id'),
...
class PermissionsSalesDetailView(PermissionsSalesView, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        """
        context['deportes'] = []
        deportes = Deportes.objects.only('pk', 'nombre').all()
...
class PermisosVentasAjax(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        json = {}
        grupos = Deportes_Grupos.objects.select_related('grupo').filter(
...
class PermissionsSalesRestrictionsFormView(PermissionsSalesView, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            json = {}
            modalidades = Modalidades.objects.only('pk', 'modalidad').filter(
...
class PermissionsSalesRestrictionsAjax(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        deporte = Deportes.objects.only('pk').get(pk=request.REQUEST.get('deporte'))
        comercializadora = Comercializadora.objects.get(pk=request.REQUEST.get('comercializadora'))
