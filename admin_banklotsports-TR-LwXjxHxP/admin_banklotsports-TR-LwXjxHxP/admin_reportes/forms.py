...
from admin_juego.models import Deportes, Encuentros, GruposApuestas, Modalidades, Temporadas
...
class FilterDeportesForm(WidgetCustomizeForms, forms.Form):
    ...
    temporada = forms.ModelChoiceField(
        required=False,
        queryset=Temporadas.objects.select_related('torneo').all(),
    ...
    def __init__(self, *args, **kwargs):
        ...
        def label_from_instance_encuentros(obj):
            ...
            campo = 'Referencia: {0} -> Fecha: {1} ->Hora: {2} -> Equipos: '.format(
                obj.pk,
                objFecha.getFecha(),
                objFecha.getHora(),
            )
        ...
        if not self.data.get('encuentro'):
            self.fields['encuentro'].queryset = Encuentros.objects.none()
        ...
        if encuentro:
            ...
            if temporada:
                ...
                if deporte:
                    ...
                    if 'temporada' in self.fields:
                        self.fields['temporada'].queryset = Temporadas.objects.filter(torneo__deporte=deporte)
...
class FilterModalidadesForm(WidgetCustomizeForms, forms.Form):
    grupo_modalidad = forms.ModelChoiceField(
        required=False,
        queryset=GruposApuestas.objects.only('nombre').all().order_by('nombre'),
        empty_label='Todos los grupos'
    )
    modalidad = forms.ModelChoiceField(
        required=False,
        queryset=Modalidades.objects.only('modalidad').all().order_by('modalidad'),
