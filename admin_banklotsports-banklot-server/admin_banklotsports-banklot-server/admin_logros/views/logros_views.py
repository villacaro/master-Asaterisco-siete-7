...
from admin_juego.models import (
    Deportes_Grupos, Encuentros, EncuentrosDetail, EncuentrosModalidades, Jugadas, JugadasInformativas, JugadorTipo,
    RestriccionesReferencias, SistemaJuego, Temporadas,
)
...
class LogrosView(MyViewBase):
    ...
    def get_queryset(self):
        ...
        if self.get_profile().codename == 'userprofile_master':
            ...
            encuentros = Encuentros.objects.all().exclude(
                horajuego__lt=now()
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        for deporte_grupo in Deportes_Grupos.objects.filter(
            ...
            if referencia:
                rangos_json = {}
                try:
                    rango = RestriccionesReferencias.objects.get(
                        deporte=deporte_grupo.deporte,
                        grupo=deporte_grupo.grupo,
                        min_ref='logro'
                    )
            ...
            for modalidad_grupo in modalidades_grupos_list.order_by('modalidad__orden'):
                ...
                encu_moda = EncuentrosModalidades.get_carefully(
                    kwargs={
                ...
                if encu_moda.modalidad_grupo.modalidad.etiqueta_ref:
                    ...
                    """
                    if RestriccionesReferencias.objects.filter(
                        deporte=deporte_grupo.deporte,
                        grupo=deporte_grupo.grupo,
                        ...
                        for encuentro in jornada.encuentros_set.filter(
                            ...
                            if not json_encuentro:
                                ...
                                for jugador_tipo in JugadorTipo.objects.filter(
                                    ...
                                    modalidad_json['jugadas'] = []
                                    detalle_encuentros = EncuentrosDetail.objects.filter(
                                        encuentro=encuentro,
                                ...
                                for deporte_grupo in Deportes_Grupos.objects.filter(
                                    ...
                                    for modalidad_grupo in modalidades_grupos_list.all(
                                        ...
                                        if modalidad_grupo.deporte_restriccion.filter(
                                            ...
                                            continue
                                        try:
                                            encu_moda = EncuentrosModalidades \
                                                .get_carefully(
                                                    kwargs={
                                                        'encuentro': encuentro,
                                                        'deporte_grupo': deporte_grupo,
