...
from admin_juego.models import (
    Condiciones, Deportes, Deportes_Grupos, Encuentros, EncuentrosModalidades, EventNotification, Jugadas,
    JugadasInformativas, RestriccionesReferencias, SistemaJuego,
)
...
class EncuentrosLogrosCreateUpdateForm(WidgetCustomizeForms, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for deporte_grupo in Deportes_Grupos.objects.filter(
            ...
            for modalidad_grupo in modalidad_grupo_list.order_by('modalidad__orden'):
                ...
                encuentro_modalidad = EncuentrosModalidades.get_carefully(
                    kwargs={
    ...
    def clean(self):
        ...
        error = 0
        for l in logros:
            jugada = Jugadas.objects.get(pk=l)
            try:
                rango = RestriccionesReferencias.objects.get(
        ...
        if error != 1:
            ...
            for r in referencias_modalidades:
                encuentro_modalidad = EncuentrosModalidades.objects.get(pk=r)
        ...
        if error != 1:
            ...
            for c in referencias_condiciones:
                jugada = Jugadas.objects.get(pk=c)
