...
from admin_juego.models import Equipos, EquiposGrupos, EquiposTemporadas, GruposJuego
...
class GruposJuegoListView(GruposJuegoView, ListView):
    ...
    def get_queryset(self):
        if self.get_filter_form().is_valid():
            ...
            gruposjuego = GruposJuego.objects.filter(
                temporada__torneo__deporte=deporte,
            )
        else:
            gruposjuego = GruposJuego.objects.none()
        return gruposjuego
...
class GruposJuegoUpdateView(GruposJuegoView, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        equipos = EquiposTemporadas.objects.filter(
    ...
    def form_valid(self, form):
        ...
        equipos_grupo_new = []
        for equipo in self.request.POST.getlist("equipo_grupo"):
            equipos_grupo = EquiposGrupos.objects.get_or_create(
                grupo=form.instance,
                equipo=Equipos.objects.get(pk=equipo)
            )[0]
            equipos_grupo_new.append(equipos_grupo.pk)

        for equipo_grupo_old in EquiposGrupos.objects.filter(
            grupo=form.instance
        ).exclude(
...
class GruposJuegoListbyTemporadaAjax(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        grupos = GruposJuego.objects.filter(
