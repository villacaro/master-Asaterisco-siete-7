...
from admin_finanzas.models import Comercializadora, Cuenta, Dia, DiaTrabajo, Movimiento, TipoMovimiento
...
class MovimientoForm(WidgetCustomizeForms, forms.ModelForm):
    ...
    def save(self, commit=True, *args, **kwargs):
        ...
        comercializadora = self.view.object_comercializadora.get_object()
        if comercializadora.user_type_codename != 'userprofile_operadora':
            if comercializadora.resumen_automatic:
                self.instance.dia = Dia.objects.get_or_create(
                    fecha=date.today()
                )[0]
            else:
