...
class Funs:
    ...
    def is_first_week_of_month():
        now = datetime.now()
        if now.day < 7 and now.weekday() < 6:
            # Antes de finalizar la primera semana, pero cuidando que
            # sea antes del domingo, porque en frecuencias
            # semanales los cortes son de lunes a domingo
            return True
        else:
            return False

    @staticmethod
    def is_first_week_of_fortnight():
        now = datetime.now()
        if now.day < 7 or (now.day > 16 and now.day < 22):
            return True
        else:
            return False

    @staticmethod
    def is_first_fortnight_of_month():
        now = datetime.now()
        if now.day < 16:
            return True
        else:
