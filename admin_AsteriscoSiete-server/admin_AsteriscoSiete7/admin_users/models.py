...
class UsersManager(models.Manager):
    ...
    def authenticate(self, username, password):
        ...
        hay una validacion que al notar que la contrase̱a es incorrecta,
        se va a una segunda verificcion con la contrase̱a anterior,
        se der correcta dicha contrase̱a con este key, se procede a crear
...
class Users(AbstractBaseUser):
    ...
    profile = models.ForeignKey(
        'admin_permisologia.Permissions',  # perfil de permisos del usuario
    ...
    def get_absolute_url(self):
        from django.urls import reverse, NoReverseMatch
        try:
            return reverse('admin_users_users_detail', kwargs={'pk': self.pk})
        except NoReverseMatch:
            return '/admin/'
    ...
    def get_token(self, is_save=True, token_len=48):
        """
        Genera un token de tama̱o 6, comprendido por letras y numeros
        """
        ...
        if not status:
            ...
            try:
                status = self.statusdetail_set.select_related(
                    'admin_status.Status').get(enddate=None).status
    ...
    def get_permissions(self, session_pk, comercializadora):
        ...
        if enlaces is None:
            ...
            exclude = []
            if profile_codename != 'userprofile_operadora':
                if comercializadora.get_object().resumen_automatic:
                    exclude = [
                        'admin_finanzas_resumenadministrativo_general',
                        'admin_finanzas_resumenadministrativo_general_import',
                    ]
                else:
                    exclude = [
                        'admin_reportes_cuadre_parley_list',
                        'admin_reportes_cuadre_parley_list_datatable',
            ...
            if self.superuser is True:
                ...
                """
                from admin_permisologia.models import Permissions
                for permisos in Permissions.objects.all().exclude(codename__in=exclude)\
                ...
                obteniendo permisos individuales asociados
                """
                for permisos in self.user_permissions.all().exclude(codename__in=exclude):
                    if permisos.profiles.filter(codename=profile_codename).exists():
                        for permiso in permisos.menu.filter(is_view=True):
                            json_menu = permiso.get_json()
                            if json_menu.get("type") == 1:
                ...
                """
                for grupo in self.groups.all():
                    for permisos in grupo.permissions.all().exclude(codename__in=exclude):
                        if permisos.profiles.filter(codename=profile_codename).exists():
                            for permiso in permisos.menu.filter(is_view=True):
                                json_menu = permiso.get_json()
                                if json_menu.get("type") == 1:
