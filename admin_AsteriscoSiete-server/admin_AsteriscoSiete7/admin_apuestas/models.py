...
class Tickets(models.Model):
    ...
    user = models.ForeignKey(
        ...
        related_name='apuestas_tickets_user',
    )
    ticket_type = models.ForeignKey(
        'admin_juego.apuesta',  # TODO: revisar modelo destino
        on_delete=models.CASCADE,
    )
    ...
    def get_absolute_url(self):
        ...
        try:
            return reverse('admin_reportes_tickets_detail', kwargs={'pk': self.pk})
    ...
    def set_new_status(self, status, initial=False):
        ...
        if not self.status_id:
            self.status = status
            self.save(update_fields=['admin_status.Status', 'updated_at'])
        else:
...
class TicketsDetail(models.Model):
    ...
    ticket = models.ForeignKey(
        'admin_apuestas.Tickets',  # TODO: revisar modelo destino
        on_delete=models.CASCADE,
    )
    monto = models.DecimalField(
        max_digits=30,
...
class TicketsDetailStatus(models.Model):
    ...
    detalle_ticket = models.ForeignKey(
        'admin_apuestas.TicketsDetail',  # TODO: revisar modelo destino
        on_delete=models.CASCADE,
    )
    created_at = models.DateTimeField(
        auto_now_add=True,
