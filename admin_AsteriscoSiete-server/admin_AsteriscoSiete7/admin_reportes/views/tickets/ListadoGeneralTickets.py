...
from admin_juego.models import ModalidadJuego_Grupos
...
from admin_reportes.forms import (
    FilterDeportesForm, FilterFechasForm, FilterModalidadesForm, FilterTicketsTiposAndStatusForm,
)
...
class ListadoTickets(MyViewBase, TemplateView):
    template_name = 'admin_reportes/tickets/listadodetickets.html'
...
class FilterListadoTickets(object):
    ...
    def set_filter(self, qs):
        ...
        if self.request.GET.get('fecha_inicio'):
            fecha_ini = self.request.GET.get('fecha_inicio') + hora_cero
        ...
        if self.request.GET.get('encuentro'):
            ...
            kwargs[prefix + '__jornada__temporadas_id'] = self.request.GET.get('temporada')
        ...
        if self.request.GET.get('condicion'):
            ...
            if self.request.GET.get('grupo_modalidad'):
                grupo_modalidad = ModalidadJuego_Grupos.objects.get(
        ...
        if self.request.GET.get('agencia'):
            ...
            kwargs[prefix + '__distribuidores_id'] = self.request.GET.get('distribuidor')

        elif self.request.GET.get('banca'):
            kwargs[prefix + '__distribuidores__banca_id'] = self.request.GET.get('banca')

        elif self.request.GET.get('bloque'):

            kwargs[prefix + '__distribuidores__banca__bloque_id'] = self.request.GET.get('bloque')
...
class ListadoTicketsAjax(BaseDatatableView):
    ...
    def prepare_results(self, qs, acarreo):
        ...
        for x, item in enumerate(qs):
            ...
            json_data.append([
                (x + 1 + acarreo),
                banca.nombre,
                '%s (%s)' % (agencia, taquilla),
                '%s' % item.fecha.strftime('%I:%M %p - %d %b'),
                '<a title="{0}" class="link" href="{1}">{2} {3}</a>'.format(
                    clonados,
                ...
                '<span class="tag2 %s">%s</span>' % (status, item.get_status())
            ])
        return json_data
    ...
    def prepare_footeresults(self, qs):
        ...
        total_tickets = qs.exclude(
            status__codename__in=['status_anulado', 'status_anulado_automatico']
        ).count()

        qs_premio = qs_ganador | qs_pagado | qs_frio

        if not (self.request.GET.get('encuentro') or self.request.GET.get(
                'temporada') or self.request.GET.get('deporte')):
            total_venta = qs_venta.aggregate(Sum('monto'))['monto__sum']
            if not total_venta:
                total_venta = Decimal(0)

            total_premio = qs_premio.aggregate(Sum('monto_premio'))['monto_premio__sum']
            if not total_premio:
