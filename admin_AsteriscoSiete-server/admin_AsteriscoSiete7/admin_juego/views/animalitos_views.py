...
from admin_juego.models import ModalidadJuego, NumeroSorteo, TipoNumeroSorteo
...
class JugadorCreateView(JugadorView, CreateView):
    ...
    def form_valid(self, form):
        ...
        if len(equipos) > 0:
            for i, equipo in enumerate(equipos):
                form.instance.equipos.add(
                    ModalidadJuego.objects.get(pk=equipo)
                )
        ...
        if self.request.POST["_save"] == "_addanother":
            ...
            equipos_queryset = ModalidadJuego.objects.filter(
...
class JugadorListView(JugadorView, ListView):
    ...
    def get_queryset(self):
        ...
        if jugador and jugador != "0" and jugador != "None":
            jugadores = NumeroSorteo.objects.filter(pk=jugador)
        elif equipo and equipo != "0" and equipo != "None":
            jugadores = NumeroSorteo.objects.filter(equipos=equipo)
        elif tipo and tipo != "0" and tipo != "None":
            jugadores = NumeroSorteo.objects.filter(tipo=tipo)
            if equipo == "0":
                jugadores = NumeroSorteo.objects.filter(equipos__isnull=True)
        elif deporte and deporte != "0" and deporte != "None":
            jugadores = NumeroSorteo.objects.filter(tipo__deporte=deporte)
        else:
            jugadores = NumeroSorteo.objects.none()
        return jugadores
...
class JugadorListbyTipoAjax(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        jugadores = NumeroSorteo.objects.filter(
...
class TipoListbyDeporteAjax(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        tipos = TipoNumeroSorteo.objects.filter(
