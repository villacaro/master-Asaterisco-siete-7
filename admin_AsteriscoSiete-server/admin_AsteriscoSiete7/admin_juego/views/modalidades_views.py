...
from admin_juego.models import TipoProducto, ModalidadJuego, ModalidadJuego_Grupos
...
class ModalidadesDeportesUpdateView(MyViewBase, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            self.object = ModalidadJuego.objects.get(pk=kwargs["pk"])
        except ModalidadJuego.DoesNotExist:
    ...
    def get_filter_form(self):
        ...
        if self.filter_form is None:
            form = modelformset_factory(
                ...
                extra=1,
                max_num=ModalidadJuego_Grupos.objects.filter(
                    modalidad=self.object
                ).count(),
            )
            self.filter_form = form(
                queryset=ModalidadJuego_Grupos.objects.filter(
                    modalidad=self.object
                )
            )
    ...
    def post(self, request, *args, **kwargs):
        for i in range(0, ModalidadJuego_Grupos.objects.filter(modalidad=self.object).count()):
            pk = request.POST.get("form-{0}-id".format(i))
            restricciones = request.POST.getlist("form-{0}-deporte_restriccion".format(i))
            modalidad_grupo = ModalidadJuego_Grupos.objects.get(
                pk=pk
            )
...
class ModalidadListbyGrupoAjax(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        modalidades = ModalidadJuego.objects.filter(
