...
from admin_juego.models import TipoNumeroSorteo
...
class JugadorTipoListView(JugadorTipoView, ListView):
    ...
    def get_queryset(self):
        if self.get_filter_form().is_valid():
            ...
            torneos = TipoNumeroSorteo.objects.filter(deporte=deporte)
        else:
            torneos = TipoNumeroSorteo.objects.none()
        return torneos
...
class JugadorTipoListbyDeporteAjax(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        jugadores_tipo = TipoNumeroSorteo.objects.filter(
