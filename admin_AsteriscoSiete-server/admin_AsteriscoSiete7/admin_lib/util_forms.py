...
from admin_juego.models import TipoProducto, ModalidadJuego, Fechas
...
class FilterDeporteForm(WidgetCustomizeForms, forms.Form):
    '''
    Formulario creado especificamente para filtrar por tipo de producto
    '''
...
class FilterTorneoForm(FilterDeporteForm):
    '''
    Formulario creado especificamente para filtrar por tipo de producto y producto
    '''
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs["view"].request.GET.get("deporte"):
            self.fields["torneo"].queryset = TipoProducto.objects.filter(
                pk=kwargs["view"].request.GET.get("deporte")
            ).only("nombre")


class FilterEquipoForm(FilterTorneoForm):
    '''
    Formulario creado especificamente para filtrar por tipo de producto, producto y modalidad
    '''
    equipo = forms.ModelChoiceField(
        queryset=ModalidadJuego.objects.only('pk', 'nombre').all(),
        required=False,
        empty_label='Seleccione un {0}'.format(ModalidadJuego._meta.verbose_name)
    )

    def __init__(self, *args, **kwargs):
        super(FilterEquipoForm, self).__init__(*args, **kwargs)
        if kwargs["view"].request.GET.get("equipo"):
            self.fields["equipo"].queryset = ModalidadJuego.objects.filter(
                pk=kwargs["view"].request.GET.get("equipo")
            )
        elif kwargs["view"].request.GET.get("torneo"):
            self.fields["equipo"].queryset = ModalidadJuego.objects.filter(
                pk=kwargs["view"].request.GET.get("torneo")
            ).only("nombre").distinct()

        elif kwargs["view"].request.GET.get("deporte"):
            self.fields["equipo"].queryset = ModalidadJuego.objects.filter(
                tipo_producto_id=kwargs["view"].request.GET.get("deporte")
            ).only("nombre")


class FilterTemporadaForm(FilterTorneoForm):
    '''
    Formulario creado especificamente para filtrar por tipo de producto y fecha
    '''
    temporada = forms.ModelChoiceField(
        queryset=Fechas.objects.filter(fechafin__gte=now().date()),
        required=False,
...
class FilterDeporteRangoFechaForm(WidgetCustomizeForms, forms.Form):
    '''
    Formulario creado especificamente para filtrar por tipo de producto y por rango de fechas
    '''
    deporte = forms.ModelChoiceField(
        queryset=TipoProducto.objects.only('pk', 'nombre').all(),
        label=TipoProducto._meta.verbose_name + ' (*)',
        required=True,
        empty_label='Seleccione un {0}'.format(TipoProducto._meta.verbose_name)
    )
    torneo = forms.ModelChoiceField(
    ...
    def clean(self):
        ...
        if deporte:
            self.fields['torneo'].queryset = TipoProducto.objects.filter(pk=deporte)
            if torneo:
...
class FilterDeporteFechaForm(WidgetCustomizeForms, forms.Form):
    '''
    Formulario creado especificamente para filtrar por tipo de producto y por fecha
    '''
    deporte = forms.ModelChoiceField(
        queryset=TipoProducto.objects.only('pk', 'nombre').all(),
        required=False,
...
class FilterDeporteJugadoresForm(FilterDeporteForm):
    '''
    Formulario creado especificamente para filtrar por modalidad de juego
    '''
    equipo = forms.ModelChoiceField(
        queryset=ModalidadJuego.objects.only('pk', 'nombre').all(),
        required=False,
